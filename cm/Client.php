...
function getIPByMAC($macAddress, $connectedList) {
	$macAddress = strtolower($macAddress);
	foreach ($connectedList as $singleClient) {
		if(!is_null($singleClient->mac)) {
			if($singleClient->mac == $macAddress) {
				return $singleClient->ip;
			}
		}
	}
	return false;
...
$unifi_connection = new Client('unifi-demo', 'DMitchell45!', 'https://demo.ui.com', 'office', '5.11.39.0', true);
$login            = $unifi_connection->login();
$results          = $unifi_connection->list_alarms(); // returns a PHP array containing alarm objects
$dvl 			  = $unifi_connection->list_clients();
...
echo "Requested in URL GET:-> ". getIPByMAC($val, $dvl);

print("Entire Data: <pre>".print_r($dvl,true)."</pre>");
